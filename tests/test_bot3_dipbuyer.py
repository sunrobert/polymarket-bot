...
def _strategy() -> Bot3DipBuyerStrategy:
...
def test_buys_up_when_ask_at_entry():
    s = _strategy()
...
def test_buys_up_when_ask_below_entry():
    s = _strategy()
    snap = _snap(up_ask=Decimal("0.20"), down_ask=Decimal("0.80"))
    intent = s.decide(snap, holds_market=False, position=None)
    assert intent is not None
    assert intent.action == "buy"


def test_skips_when_ask_above_entry():
    s = _strategy()
    snap = _snap(up_ask=Decimal("0.40"), down_ask=Decimal("0.60"))
    assert s.decide(snap, holds_market=False, position=None) is None


def test_sells_when_implied_bid_at_exit():
    # implied up bid = 1 - down_ask = 1 - 0.45 = 0.55
    s = _strategy()
...
def test_holds_when_implied_bid_below_exit():
    # implied up bid = 1 - 0.50 = 0.50, below 0.55
    s = _strategy()
...
def test_does_not_rebuy_while_holding():
    ...
    pos = _position()
    intent = s.decide(snap, holds_market=True, position=pos)
    # Position present, implied bid (0.20) below exit — should hold, not buy again.
